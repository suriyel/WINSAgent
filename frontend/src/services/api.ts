...
/**
 * Submit missing-params decision and handle SSE stream response.
 * Returns an AbortController to cancel the stream if needed.
 */
export function submitParamsDecision(
  executionId: string,
  toolName: string,
  ...
  (async () => {
    try {
      const res = await fetch(`${BASE}/params/${executionId}/decide`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action, tool_name: toolName, params: editedParams }),
        signal: controller.signal,
      });
