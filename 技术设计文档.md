...
graph TD
    subgraph 接入层
        UI[React 前端工作台] --> SSE[SSE EventSource]
        UI --> REST[REST API]
    end
    ...
    subgraph 业务逻辑层
        ...
        AgentCore --> SubAgentMgr[SubAgent 管理器]
        AgentCore --> KnowledgeRetriever[领域知识检索器]
    end

    subgraph 数据层
        ToolRegistry --> LegacyAPI[存量系统 API]
        KnowledgeRetriever --> FAISSIndex[FAISS 向量索引]
...
graph LR
    ...
    Backend --> MainAgent[主Agent]
    MainAgent --> |middleware| HITLMiddleware[HITL中间件]
    MainAgent --> |middleware| TodoMiddleware[TODO中间件]
    MainAgent --> |tool call| ToolRegistry[Tool注册中心]
    MainAgent --> |tool call| SubAgent[SubAgent-as-Tool]
    MainAgent --> |retrieval| FAISSRetriever[FAISS检索器]
    SubAgent --> |tool call| ToolRegistry
    ToolRegistry --> LegacySystem[存量系统]
...
sequenceDiagram
    ...
    U->>API: POST /api/chat (用户输入)
    API->>Agent: agent.stream({messages})
    Agent->>MW: TodoMiddleware.before_model()
    MW-->>SSE: todo.state 事件 (初始化TODO)
    Agent->>LLM: 发送消息 + Tool描述
    LLM-->>SSE: thinking 事件 (Token流)

    alt 识别到专业术语
        Agent->>FAISS: 领域知识检索
        FAISS-->>Agent: 返回相关文档片段
        Agent->>LLM: 补充领域知识上下文
    end

    LLM-->>Agent: 选择Tool + 参数
    SSE-->>U: tool.call 事件 (Tool名称+参数)

    alt HumanInTheLoop触发
        MW->>U: 阻塞等待用户确认
    ...
    Agent->>Tool: 执行Tool调用
    Tool-->>Agent: 返回结果
    SSE-->>U: tool.result 事件
    MW-->>SSE: todo.state 事件 (更新步骤状态)

    Agent->>LLM: 处理Tool结果
    LLM-->>SSE: thinking 事件
    LLM-->>Agent: 最终回复
    SSE-->>U: message 事件 (完整回复)
```
...
sequenceDiagram
    ...
    Agent->>HITL: Tool调用前拦截
    HITL->>SSE: hitl.pending 事件 (Tool名+参数+schema)
    SSE->>U: 展示配置卡片 (可编辑参数)

    alt approve
        U->>SSE: hitl.approve (原始参数)
        SSE->>HITL: 继续执行
        HITL->>Agent: 放行Tool调用
    else edit
        U->>SSE: hitl.edit (修改后参数)
        SSE->>HITL: 使用新参数
        HITL->>Agent: 以修改参数执行Tool
    else reject
        U->>SSE: hitl.reject
        SSE->>HITL: 终止
        HITL->>Agent: 抛出异常，任务失败
    end
```
...
stateDiagram-v2
    [*] --> pending: 初始化
    pending --> in_progress: Agent开始执行该步骤
    in_progress --> completed: 步骤执行成功
    in_progress --> failed: 步骤执行失败
    failed --> [*]: 整个任务终止
    completed --> [*]: 所有步骤完成
    in_progress --> replan: LLM决定调整计划
    replan --> pending: 重新规划步骤
```
...
erDiagram
    ...
    TOOL_DEFINITION {
        ...
        string category "耗时/复杂/外部调用"
        boolean requires_hitl
        json dependencies "依赖的其他Tool名称列表"
    }

    KNOWLEDGE_BASE {
        string id PK
        string name "术语表/设计文档"
...
sequenceDiagram
    ...
    Note over FE: 收到 hitl.pending 事件
...
```mermaid
graph TD
    App[App] --> Layout[WorkstationLayout 三栏布局]
    Layout --> Sidebar[ConversationSidebar 左侧边栏]
    Layout --> ChatArea[ChatArea 中间聊天区]
    Layout --> TaskPanel[TaskPanel 右侧任务面板]
    ...
    MessageBubble --> TodoStepper[TodoStepper 嵌入式TODO]
    MessageBubble --> ToolCallCard[ToolCallCard Tool调用卡片]
    MessageBubble --> ThinkingIndicator[ThinkingIndicator 思考中]

    TodoStepper --> StepItem[StepItem 步骤项]
    StepItem --> StatusIcon[StatusIcon 状态图标]
    StepItem --> ProgressBar[ProgressBar 进度条]

    ToolCallCard --> HITLModal[HITLConfigModal 配置弹窗]
    HITLModal --> DynamicForm[DynamicForm 动态表单]
    HITLModal --> ActionButtons[ActionButtons approve/edit/reject]

    TaskPanel --> TaskList[TaskList]
    TaskPanel --> TaskCard[TaskCard 任务卡片]
...
graph LR
    subgraph 知识管理
        ...
        Splitter1 --> Embed1[Embedding模型]
        Splitter2 --> Embed2[Embedding模型]
...
```mermaid
gantt
    title WINS Agent 工作台开发里程碑
    dateFormat  YYYY-MM-DD
    axisFormat  %m/%d

    section M1: 最小可行原型 (MVP)
    项目初始化与框架搭建         :m1_1, 2026-02-03, 3d
    LangChain Agent 核心集成     :m1_2, after m1_1, 4d
    Tool注册中心与示例Tool       :m1_3, after m1_2, 3d
    SSE流式输出（后端）          :m1_4, after m1_2, 3d
    前端基础框架与聊天界面        :m1_5, 2026-02-03, 5d
    前后端SSE联调               :m1_6, after m1_4, 2d

    section M2: 功能对标 (Feature Complete)
    HumanInTheLoop中间件集成     :m2_1, after m1_6, 3d
    TodoListMiddleware自定义开发  :m2_2, after m1_6, 4d
    HITL动态配置UI              :m2_3, after m2_1, 3d
    TODO可视化组件              :m2_4, after m2_2, 3d
    FAISS领域知识检索           :m2_5, after m2_2, 4d
    SubAgent-as-Tool            :m2_6, after m2_5, 3d
    任务进度面板                :m2_7, after m2_4, 2d

    section M3: 生产就绪 (Production Ready)
    UCD设计规范UI精修           :m3_1, after m2_7, 4d
    MySQL持久化替换InMemory     :m3_2, after m2_6, 3d
    知识库增量更新机制           :m3_3, after m3_2, 3d
    API鉴权与安全加固           :m3_4, after m3_2, 2d
    集成测试与Bug修复           :m3_5, after m3_3, 4d
    部署文档与环境配置           :m3_6, after m3_5, 2d
```
...
| 模块 | 说明 | 开发量评估 |
| :--- | :--- | :--- |
| ~~TodoListMiddleware~~ | ~~自定义开发~~ → **LangChain 1.2.5 原生提供**（`langchain.agents.middleware.todo.TodoListMiddleware`），直接集成即可。 | 无需开发 |
